...
@app.route('/api/v1/students/gap-analysis', methods=['POST'])
def gap_analysis():
    """
    Endpoint 6: Analyze gap between student score and target programs

    Request body:
    {
        "current_score": 10.5,
        "target_degrees": ["21079", "11043", "41058"]
    }
    """
    try:
        data = request.get_json()
        ...
        target_codes = data.get('target_degrees', [])

        if current_score is None or current_score < 0 or current_score > 14:
            return jsonify({'error': 'Current score must be between 0 and 14'}), 400

        if not target_codes:
            return jsonify({'error': 'Must provide at least one target degree code'}), 400

        # Find target degrees and calculate gaps
        results = []
        for code in target_codes:
            for degree in DEGREES_DATA:
                if degree['code'] == code:
                    gap = degree['cut_off_score'] - current_score
                    status = 'Reachable' if gap <= 0 else 'Above current score'

                    results.append({
                        'degree': degree,
                        'gap': round(gap, 3),
                        'status': status,
                        'points_needed': round(abs(gap), 3) if gap > 0 else 0
                    })
                    break

        # Find similar alternatives (same field, within reach)
        alternatives = []
        if results:
            target_fields = list(set([r['degree']['field'] for r in results]))

            for degree in DEGREES_DATA:
                if degree['field'] in target_fields and degree['cut_off_score'] <= current_score:
                    if degree['code'] not in target_codes:
                        alternatives.append(degree)

            # Sort alternatives by score (descending)
            alternatives.sort(key=lambda x: x['cut_off_score'], reverse=True)
...
if __name__ == '__main__':
    ...
    print("  POST   /api/v1/students/gap-analysis")
